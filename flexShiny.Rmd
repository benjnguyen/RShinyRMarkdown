...
# If we wanted to issue a button, uncomment here
...
# If we used the button under the UI, we would need an eventReactive
# This is one such way to use an eventReactive
# The benefit to an eventReactive is to prevent the app from computing intermediate changes
# that are not the final change that the user wants to observe.
# Note that we should use an eventReactive everywhere that performs a calculation w.r.t. a user input.
...
# We calculate this because we want to compare it to our portfolio's Sharpe Ratio
# This serves as an alternate reality where we invest in the S&P500 as opposed to the stocks that we chose.
...
```{r}
# Note that the way we name our plots is not from an assigment operator.
dygraphOutput("dygraphDollarGrowth")
# Also note that the rendering function uses a reactive() as a parameter.
